...
TEST(execute_query_includes_relationships) {
    FIXTURE_ARENA_SETUP();
    ...
    SysmlRelationship rel = {
        .id = "bind1",
        .kind = SYSML_KIND_REL_BIND,
        .source = "Pkg::A",
        .target = "Pkg::B",
    };
    SysmlRelationship *rel_ptrs[1] = {&rel};
    model.relationships = rel_ptrs;
    model.relationship_count = 1;

    SysmlSemanticModel *models[] = {&model};
    const char *patterns[] = {"Pkg::A", "Pkg::B"};
    Sysml2QueryPattern *p = sysml2_query_parse_multi(patterns, 2, &arena);
    Sysml2QueryResult *result = sysml2_query_execute(p, models, 1, &arena);
    ...
    ASSERT_EQ(result->relationships[0]->kind, SYSML_KIND_REL_BIND);
    ASSERT_STR_EQ(result->relationships[0]->source, "Pkg::A");
    ASSERT_STR_EQ(result->relationships[0]->target, "Pkg::B");

    FIXTURE_ARENA_TEARDOWN();
}
TEST(execute_query_prefers_resolved_relationship_endpoints) {
    FIXTURE_ARENA_SETUP();

    SysmlSemanticModel structure = {0};
    SysmlNode structure_nodes[1] = {
        {.id = "DemoStruct::vehicleParam", .name = "vehicleParam", .kind = SYSML_KIND_PART_USAGE, .parent_id = "DemoStruct"},
    };
    SysmlNode *structure_node_ptrs[1] = {&structure_nodes[0]};
    structure.elements = structure_node_ptrs;
    ...
    SysmlRelationship rel = {
        ...
        .source = "RootReq",
        .target = "vehicleParam",
        .resolved_source = "DemoReqs::RootReq",
        .resolved_target = "DemoStruct::vehicleParam",
    };
    SysmlRelationship *structure_rel_ptrs[1] = {&rel};
    structure.relationships = structure_rel_ptrs;
    structure.relationship_count = 1;

    SysmlSemanticModel reqs = {0};
    SysmlNode req_nodes[1] = {
        {.id = "DemoReqs::RootReq", .name = "RootReq", .kind = SYSML_KIND_REQUIREMENT_DEF, .parent_id = "DemoReqs"},
    };
    SysmlNode *req_node_ptrs[1] = {&req_nodes[0]};
    reqs.elements = req_node_ptrs;
    reqs.element_count = 1;

    SysmlSemanticModel *models[2] = {&structure, &reqs};
    const char *patterns[] = {"DemoReqs::RootReq", "DemoStruct::vehicleParam"};
    Sysml2QueryPattern *p = sysml2_query_parse_multi(patterns, 2, &arena);
    Sysml2QueryResult *result = sysml2_query_execute(p, models, 2, &arena);

    ASSERT_NOT_NULL(result);
    ASSERT_EQ(result->element_count, 2);
    ASSERT_EQ(result->relationship_count, 1);
    ASSERT_EQ(result->relationships[0]->kind, SYSML_KIND_REL_SATISFY);
    ASSERT_STR_EQ(result->relationships[0]->source, "RootReq");
    ASSERT_STR_EQ(result->relationships[0]->target, "vehicleParam");
    ASSERT_STR_EQ(result->relationships[0]->resolved_source, "DemoReqs::RootReq");
    ASSERT_STR_EQ(result->relationships[0]->resolved_target, "DemoStruct::vehicleParam");
...
int main(void) {
    ...
    RUN_TEST(execute_query_includes_relationships);
    RUN_TEST(result_contains);
    RUN_TEST(execute_query_prefers_resolved_relationship_endpoints);
