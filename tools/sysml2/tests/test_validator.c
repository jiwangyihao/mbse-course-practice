...
TEST(validate_multi_resolves_imported_relationship_endpoints) {
    FIXTURE_SETUP();
    Sysml2DiagContext diag_ctx;
    sysml2_diag_context_init(&diag_ctx, &arena);

    SysmlSemanticModel structure = {0};
    SysmlNode structure_pkg = {
        .id = "DemoStruct",
        .name = "DemoStruct",
        .kind = SYSML_KIND_PACKAGE,
    };
    SysmlNode vehicle = {
        .id = "DemoStruct::vehicleParam",
        .name = "vehicleParam",
        .kind = SYSML_KIND_PART_USAGE,
        .parent_id = "DemoStruct",
    };
    SysmlNode *structure_nodes[2] = {&structure_pkg, &vehicle};
    structure.elements = structure_nodes;
    structure.element_count = 2;

    SysmlRelationship satisfy = {
        .id = "rel1",
        .kind = SYSML_KIND_REL_SATISFY,
        .owner_scope = "DemoStruct::vehicleParam",
        .source = "DemoReqs::RootReq",
        .target = "DemoStruct::vehicleParam",
    };

    SysmlRelationship *structure_relationships[1] = {&satisfy};
    structure.relationships = structure_relationships;
    structure.relationship_count = 1;

    SysmlSemanticModel reqs = {0};
    SysmlNode reqs_pkg = {
        .id = "DemoReqs",
        .name = "DemoReqs",
        .kind = SYSML_KIND_PACKAGE,
    ...
    SysmlNode root_req = {
        .id = "DemoReqs::RootReq",
        .name = "RootReq",
        .kind = SYSML_KIND_REQUIREMENT_DEF,
        .parent_id = "DemoReqs",
    };
    SysmlNode *req_nodes[2] = {&reqs_pkg, &root_req};
    reqs.elements = req_nodes;
    reqs.element_count = 2;

    SysmlSemanticModel *models[2] = {&structure, &reqs};
    Sysml2ValidationOptions opts = SYSML_VALIDATION_OPTIONS_DEFAULT;
    Sysml2Result result = sysml2_validate_multi(models, 2, &diag_ctx, &arena, &intern, &opts);

    ASSERT_EQ(result, SYSML2_OK);
    ASSERT_NOT_NULL(satisfy.resolved_source);
    ASSERT_NOT_NULL(satisfy.resolved_target);
    ASSERT_STR_EQ(satisfy.resolved_source, "DemoReqs::RootReq");
    ASSERT_STR_EQ(satisfy.resolved_target, "DemoStruct::vehicleParam");

    FIXTURE_TEARDOWN();
}

TEST(validate_resolves_forward_local_relationship_endpoints) {
    FIXTURE_SETUP();
    Sysml2DiagContext diag_ctx;
    sysml2_diag_context_init(&diag_ctx, &arena);
    ...
    SysmlNode req = {
        .id = "ForwardLocal::q::req",
        .name = "req",
        .kind = SYSML_KIND_REQUIREMENT_USAGE,
        .parent_id = "ForwardLocal::q",
    };
    SysmlNode *nodes[4] = {&pkg, &q, &p, &req};
    ...
    SysmlRelationship satisfy = {
        .id = "rel2",
        .kind = SYSML_KIND_REL_SATISFY,
        .owner_scope = "ForwardLocal::q",
        .source = "req",
        .target = "p",
    };
    SysmlRelationship *relationships[1] = {&satisfy};
    model.relationships = relationships;
    model.relationship_count = 1;

    Sysml2ValidationOptions opts = SYSML_VALIDATION_OPTIONS_DEFAULT;
    Sysml2Result result = sysml2_validate(&model, &diag_ctx, NULL, &arena, &intern, &opts);

    ASSERT_EQ(result, SYSML2_OK);
    ASSERT_NOT_NULL(satisfy.resolved_source);
    ASSERT_NOT_NULL(satisfy.resolved_target);
    ASSERT_STR_EQ(satisfy.resolved_source, "ForwardLocal::q::req");
    ASSERT_STR_EQ(satisfy.resolved_target, "ForwardLocal::p");

    FIXTURE_TEARDOWN();
}

TEST(validate_ambiguous_imported_relationship_source_stays_unresolved) {
    FIXTURE_SETUP();
    Sysml2DiagContext diag_ctx;
    sysml2_diag_context_init(&diag_ctx, &arena);
    ...
    SysmlImport import_a = {
        .id = "impA",
        .kind = SYSML_KIND_IMPORT_ALL,
        .target = "PkgA",
        .owner_scope = "DemoStruct",
    };
    SysmlImport import_b = {
        .id = "impB",
        .kind = SYSML_KIND_IMPORT_ALL,
        .target = "PkgB",
        .owner_scope = "DemoStruct",
    };
    SysmlImport *imports[2] = {&import_a, &import_b};
    structure.imports = imports;
    structure.import_count = 2;
    ...
    SysmlSemanticModel pkg_a = {0};
    SysmlNode pkg_a_root = {
        .id = "PkgA",
        .name = "PkgA",
        .kind = SYSML_KIND_PACKAGE,
    };
    ...
    SysmlNode *pkg_a_nodes[2] = {&pkg_a_root, &req_a};
    pkg_a.elements = pkg_a_nodes;
    pkg_a.element_count = 2;

    SysmlSemanticModel pkg_b = {0};
    SysmlNode pkg_b_root = {
        .id = "PkgB",
        .name = "PkgB",
        .kind = SYSML_KIND_PACKAGE,
    };
    ...
    SysmlNode *pkg_b_nodes[2] = {&pkg_b_root, &req_b};
    pkg_b.elements = pkg_b_nodes;
    pkg_b.element_count = 2;

    SysmlSemanticModel *models[3] = {&structure, &pkg_a, &pkg_b};
    Sysml2ValidationOptions opts = SYSML_VALIDATION_OPTIONS_DEFAULT;
    Sysml2Result result = sysml2_validate_multi(models, 3, &diag_ctx, &arena, &intern, &opts);

    ASSERT_EQ(result, SYSML2_OK);
    ASSERT_NULL(satisfy.resolved_source);
    ASSERT_NOT_NULL(satisfy.resolved_target);
    ASSERT_STR_EQ(satisfy.resolved_target, "DemoStruct::vehicleParam");
...
TEST(validate_duplicate_imports_do_not_make_relationship_ambiguous) {
    FIXTURE_SETUP();
    Sysml2DiagContext diag_ctx;
    sysml2_diag_context_init(&diag_ctx, &arena);
...
TEST(validate_local_symbol_shadows_import_for_relationship_resolution) {
    FIXTURE_SETUP();
    Sysml2DiagContext diag_ctx;
    sysml2_diag_context_init(&diag_ctx, &arena);
    ...
    SysmlNode *structure_nodes[3] = {&structure_pkg, &local_req, &vehicle};
    structure.elements = structure_nodes;
    structure.element_count = 3;

    SysmlImport import_reqs = {
        .id = "impLocalShadow",
        .kind = SYSML_KIND_IMPORT_ALL,
        .target = "DemoReqs",
        .owner_scope = "DemoStruct",
    };
    SysmlImport *imports[1] = {&import_reqs};
    structure.imports = imports;
    ...
    SysmlNode imported_req = {
        .id = "DemoReqs::RootReq",
        .name = "RootReq",
        .kind = SYSML_KIND_REQUIREMENT_DEF,
        .parent_id = "DemoReqs",
    };
    SysmlNode *req_nodes[2] = {&reqs_pkg, &imported_req};
    reqs.elements = req_nodes;
    reqs.element_count = 2;
    ...
    ASSERT_NOT_NULL(satisfy.resolved_source);
    ASSERT_NOT_NULL(satisfy.resolved_target);
    ASSERT_STR_EQ(satisfy.resolved_source, "DemoStruct::RootReq");
    ASSERT_STR_EQ(satisfy.resolved_target, "DemoStruct::vehicleParam");
...
int main(void) {
    ...
    RUN_TEST(validate_multi_resolves_imported_relationship_endpoints);
    RUN_TEST(validate_resolves_forward_local_relationship_endpoints);
    RUN_TEST(validate_ambiguous_imported_relationship_source_stays_unresolved);
    RUN_TEST(validate_duplicate_imports_do_not_make_relationship_ambiguous);
    RUN_TEST(validate_local_symbol_shadows_import_for_relationship_resolution);
