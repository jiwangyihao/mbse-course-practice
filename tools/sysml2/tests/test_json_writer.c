...
TEST(json_write_with_relationships) {
    ...
    ASSERT(strstr(output, "\"ownerScope\"") != NULL);
    ASSERT(strstr(output, "\"sourceRaw\"") != NULL);
    ASSERT(strstr(output, "\"targetRaw\"") != NULL);

    free(output);
    FIXTURE_TEARDOWN();
}

TEST(json_write_with_result_expression) {
    FIXTURE_SETUP();

    SysmlSemanticModel *model = create_test_model(&arena, &intern, "test.sysml");

    SysmlNode *node = SYSML2_ARENA_NEW(&arena, SysmlNode);
    node->id = sysml2_intern(&intern, "Pkg::MassConstraint");
    node->name = sysml2_intern(&intern, "MassConstraint");
    node->kind = SYSML_KIND_CONSTRAINT_DEF;
    node->parent_id = sysml2_intern(&intern, "Pkg");
    node->result_expression = sysml2_intern(&intern, "totalMass == componentMasses");
    ...
    ASSERT(strstr(output, "\"resultExpression\"") != NULL);
    ASSERT(strstr(output, "totalMass == componentMasses") != NULL);
...
int main(void) {
    ...
    RUN_TEST(json_write_with_result_expression);
