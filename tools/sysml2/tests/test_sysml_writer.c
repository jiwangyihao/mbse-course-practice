...
static const SysmlNode *find_node_by_id(const SysmlSemanticModel *model, const char *id) {
    if (!model || !id) return NULL;

    for (size_t i = 0; i < model->element_count; i++) {
        if (model->elements[i] && model->elements[i]->id && strcmp(model->elements[i]->id, id) == 0) {
...
static const SysmlRelationship *find_relationship(
    const SysmlSemanticModel *model,
    SysmlNodeKind kind,
    const char *source,
    const char *target
) {
    if (!model || !source || !target) return NULL;

    for (size_t i = 0; i < model->relationship_count; i++) {
        const SysmlRelationship *rel = model->relationships[i];
        if (!rel) continue;
        if (rel->kind != kind) continue;
        if (!rel->source || !rel->target) continue;
        if (strcmp(rel->source, source) == 0 && strcmp(rel->target, target) == 0) {
            return rel;
        }
    }
...
TEST(sysml_satisfy_relationship_emitted) {
    FIXTURE_SETUP();
    ...
    const SysmlRelationship *rel = find_relationship(
        model,
    ...
    ASSERT_NOT_NULL(rel);
    ASSERT_STR_EQ(rel->owner_scope, "TestPkg");

    FIXTURE_TEARDOWN();
}

TEST(sysml_constraint_bind_relationships_emitted) {
    FIXTURE_SETUP();
    ...
    const SysmlRelationship *bind_total_mass = find_relationship(
        model,
        SYSML_KIND_REL_BIND,
        "massConstraint.totalMass",
        "mass"
    );
    ASSERT_NOT_NULL(bind_total_mass);
    ASSERT_STR_EQ(bind_total_mass->owner_scope, "TestPkg::vehicle");

    const SysmlRelationship *bind_component = find_relationship(
        model,
        SYSML_KIND_REL_BIND,
        "massConstraint.componentMasses",
        "engine.mass"
    );
    ASSERT_NOT_NULL(bind_component);
    ASSERT_STR_EQ(bind_component->owner_scope, "TestPkg::vehicle");

    const SysmlNode *constraint = find_node_by_id(model, "TestPkg::MassConstraint");
    ASSERT_NOT_NULL(constraint);
    ASSERT_NOT_NULL(constraint->result_expression);
    ASSERT_STR_EQ(constraint->result_expression, "totalMass == componentMasses");

    FIXTURE_TEARDOWN();
}

TEST(sysml_forward_satisfy_relationship_stays_lexical) {
    FIXTURE_SETUP();
    ...
    ASSERT_NOT_NULL(rel);
    ASSERT_STR_EQ(rel->owner_scope, "ForwardLocal::q");
    ASSERT_NULL(rel->resolved_source);
    ASSERT_NULL(rel->resolved_target);
...
TEST(sysml_succession_relationship_emitted) {
    FIXTURE_SETUP();
    ...
    ASSERT_NOT_NULL(rel);
    ASSERT_STR_EQ(rel->owner_scope, "TestPkg::Flow");
...
int main(void) {
    ...
    RUN_TEST(sysml_satisfy_relationship_emitted);
    RUN_TEST(sysml_constraint_bind_relationships_emitted);
    RUN_TEST(sysml_succession_relationship_emitted);
    RUN_TEST(sysml_forward_satisfy_relationship_stays_lexical);
