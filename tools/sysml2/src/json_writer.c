...
static void write_relationship(JsonWriter *w, const SysmlRelationship *rel) {
    ...
    if (rel->resolved_target) {
        write_string_field(w, "resolvedTarget", rel->resolved_target, true);
    }
...
Sysml2Result sysml2_json_write_string(
    ...
    char *buffer = NULL;
    size_t size = 0;
    FILE *memstream = sysml2_memstream_open(&buffer, &size);
    if (!memstream) {
        return SYSML2_ERROR_OUT_OF_MEMORY;
    }

    Sysml2Result result = sysml2_json_write(model, memstream, options);
    Sysml2Result close_result = sysml2_memstream_close(memstream, &buffer, &size);

    if (result == SYSML2_OK && close_result == SYSML2_OK) {
        *out_str = buffer;
        return SYSML2_OK;
    }
    ...
    free(buffer);
    *out_str = NULL;
    return result != SYSML2_OK ? result : close_result;
}
