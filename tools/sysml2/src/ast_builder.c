...
static const char *trim_span_start(const char *start, const char *end) {
...
static const char *trim_span_end(const char *start, const char *end) {
...
static const char *intern_trimmed_span(SysmlBuildContext *ctx, const char *start, const char *end) {
    if (!ctx || !start || !end || end < start) return NULL;

    start = trim_span_start(start, end);
    end = trim_span_end(start, end);
...
static bool parse_satisfy_relationship(
    SysmlBuildContext *ctx,
    const char *text,
    size_t len,
    ...
    if (!ctx || !text || !out_source || !out_target) return false;
    ...
    *out_target = NULL;
    ...
    const char *body = memchr(start, '{', len);
    if (body) {
        end = body;
    }
    end = trim_span_end(start, end);
    ...
    cursor = trim_span_start(cursor, end);
    if ((size_t)(end - cursor) >= 6 && strncmp(cursor, "assert", 6) == 0) {
        cursor += 6;
        cursor = trim_span_start(cursor, end);
    }
    if ((size_t)(end - cursor) >= 3 && strncmp(cursor, "not", 3) == 0) {
        cursor += 3;
        cursor = trim_span_start(cursor, end);
    }
    if ((size_t)(end - cursor) < 7 || strncmp(cursor, "satisfy", 7) != 0) {
        return false;
    }
    cursor += 7;
    cursor = trim_span_start(cursor, end);
    if ((size_t)(end - cursor) >= 11 && strncmp(cursor, "requirement", 11) == 0) {
        cursor += 11;
        cursor = trim_span_start(cursor, end);
    }

    const char *by = strstr(cursor, " by ");
    if (!by || by >= end) {
        return false;
    }

    const char *source_end = trim_span_end(cursor, by);
    const char *target_start = by + 4;
    const char *target_end = end;

    const char *source = intern_trimmed_span(ctx, cursor, source_end);
    const char *target = intern_trimmed_span(ctx, target_start, target_end);
    if (!source || !target) {
        return false;
    }
...
static bool parse_succession_relationship(
    SysmlBuildContext *ctx,
    const char *text,
    const char **out_source,
    const char **out_target
    ...
    *out_target = NULL;
    ...
    const char *then = strstr(start, " then ");
    if (!then || then >= end) {
        return false;
    }

    const char *source = intern_trimmed_span(ctx, start, then);
    const char *target = intern_trimmed_span(ctx, then + 6, end);
    if (!source || !target) {
        return false;
    }
...
void sysml2_capture_bind(
    ...
    build_stmt_relationship(ctx, SYSML_KIND_REL_BIND, stmt->source.target, stmt->target.target);
}
...
void sysml2_capture_succession(
    ...
    if (stmt->source.target && stmt->target.target) {
        build_stmt_relationship(ctx, SYSML_KIND_REL_SUCCESSION, stmt->source.target, stmt->target.target);
    } else {
        const char *rel_source = NULL;
        const char *rel_target = NULL;
        if (parse_succession_relationship(ctx, stmt->source.target, &rel_source, &rel_target)) {
            build_stmt_relationship(ctx, SYSML_KIND_REL_SUCCESSION, rel_source, rel_target);
        }
    }
}
...
void sysml2_capture_satisfy(SysmlBuildContext *ctx, const char *text, size_t len) {
    ...
    if (parse_satisfy_relationship(ctx, text, len, &source, &target)) {
        build_stmt_relationship(ctx, SYSML_KIND_REL_SATISFY, source, target);
    }
    sysml2_build_clear_pending_modifiers(ctx);
}
