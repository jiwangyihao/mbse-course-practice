...
static void pass1_build_symtab(
    ...
    for (size_t i = 0; i < model->import_count; i++) {
        ...
        Sysml2ImportEntry *entry = sysml2_arena_alloc(vctx->symtab->arena, sizeof(Sysml2ImportEntry));
        if (entry) {
            entry->target = extract_import_namespace(imp->target, vctx->symtab->arena);
...
#define SYSML2_RELATIONSHIP_RESOLVE_MAX_DEPTH 64

static const char *relationship_final_segment(const char *qualified_name) {
    if (!qualified_name) return NULL;

    const char *last_sep = NULL;
    for (const char *p = qualified_name; p[0] && p[1]; p++) {
        if (p[0] == ':' && p[1] == ':') {
            last_sep = p;
        }
    }

    return last_sep ? last_sep + 2 : qualified_name;
}

static bool relationship_candidate_add(RelationshipCandidateSet *set, Sysml2Symbol *sym) {
    if (!set || !sym || !sym->qualified_id) return true;

    for (size_t i = 0; i < set->count; i++) {
        if (strcmp(set->items[i]->qualified_id, sym->qualified_id) == 0) {
            return true;
        }
    }
    ...
    if (set->count >= set->capacity) {
        ...
        Sysml2Symbol **new_items = realloc(set->items, new_capacity * sizeof(Sysml2Symbol *));
...
static bool relationship_scope_seen(const Sysml2Scope *const *visited, size_t count, const Sysml2Scope *scope) {
...
static bool collect_relationship_import_candidates_depth(
    Sysml2SymbolTable *symtab,
    const Sysml2Scope *scope,
    const char *name,
    const Sysml2Scope **visited,
    size_t visited_count,
    RelationshipCandidateSet *set
) {
    if (!symtab || !scope || !name || !visited || !set) return true;
    if (visited_count >= SYSML2_RELATIONSHIP_RESOLVE_MAX_DEPTH) return true;
    if (relationship_scope_seen(visited, visited_count, scope)) return true;
    ...
    for (Sysml2ImportEntry *imp = scope->imports; imp; imp = imp->next) {
        switch (imp->import_kind) {
            case SYSML_KIND_IMPORT: {
                const char *imported_name = relationship_final_segment(imp->target);
                if (imported_name && strcmp(imported_name, name) == 0) {
                    const char *last_sep = NULL;
                    for (const char *p = imp->target; p[0] && p[1]; p++) {
                        if (p[0] == ':' && p[1] == ':') {
                            last_sep = p;
                        }
                    }
                    if (last_sep) {
                        size_t parent_len = (size_t)(last_sep - imp->target);
                        size_t local_len = strlen(last_sep + 2);
                        char *parent = sysml2_arena_alloc(symtab->arena, parent_len + 1);
                        char *local = sysml2_arena_alloc(symtab->arena, local_len + 1);
                        if (!parent || !local) return false;

                        memcpy(parent, imp->target, parent_len);
                        parent[parent_len] = '\0';
                        memcpy(local, last_sep + 2, local_len + 1);

                        Sysml2Scope *target_scope = sysml2_symtab_get_or_create_scope(symtab, parent);
                        Sysml2Symbol *sym = sysml2_symtab_lookup(target_scope, local);
                        if (sym && strcmp(sym->qualified_id, imp->target) == 0) {
                            if (!relationship_candidate_add(set, sym)) return false;
                        }
                    }
                }
                break;
            ...
            case SYSML_KIND_IMPORT_RECURSIVE: {
                Sysml2Scope *target_scope = sysml2_symtab_get_or_create_scope(symtab, imp->target);
                if (target_scope) {
                    Sysml2Symbol *sym = sysml2_symtab_lookup(target_scope, name);
                    if (!relationship_candidate_add(set, sym)) return false;

                    if (!collect_relationship_import_candidates_depth(
                            symtab, target_scope, name, visited, visited_count, set)) {
                        return false;
                    }
                }
                break;
...
static Sysml2Symbol *resolve_unique_simple_relationship_endpoint(
    ValidationContext *vctx,
    const Sysml2Scope *scope,
    const char *owner_scope,
    const char *raw
) {
    if (!vctx || !raw) return NULL;

    bool is_owner_self = false;
    if (owner_scope) {
        const char *scope_name = strrchr(owner_scope, ':');
        scope_name = scope_name ? scope_name + 1 : owner_scope;
        while (*scope_name == ':') scope_name++;
        is_owner_self = strcmp(raw, scope_name) == 0;
    }

    bool first_tier = true;
    for (const Sysml2Scope *s = scope ? scope : vctx->symtab->root_scope; s; s = s->parent) {
        RelationshipCandidateSet direct_candidates = {0};

        if (first_tier && is_owner_self) {
            Sysml2Symbol *self_sym = sysml2_symtab_resolve(
                vctx->symtab, vctx->symtab->root_scope, owner_scope);
            if (!relationship_candidate_add(&direct_candidates, self_sym)) {
                free(direct_candidates.items);
                return NULL;
            }
        }

        Sysml2Symbol *direct = sysml2_symtab_lookup(s, raw);
        if (!relationship_candidate_add(&direct_candidates, direct)) {
            free(direct_candidates.items);
            return NULL;
        }

        if (direct_candidates.count > 0) {
            Sysml2Symbol *resolved = direct_candidates.count == 1 ? direct_candidates.items[0] : NULL;
            free(direct_candidates.items);
            return resolved;
        }
        free(direct_candidates.items);

        RelationshipCandidateSet import_candidates = {0};
        const Sysml2Scope *visited[SYSML2_RELATIONSHIP_RESOLVE_MAX_DEPTH] = {0};
        if (!collect_relationship_import_candidates_depth(
                vctx->symtab, s, raw, visited, 0, &import_candidates)) {
            free(import_candidates.items);
            return NULL;
        }

        if (import_candidates.count > 0) {
            Sysml2Symbol *resolved = import_candidates.count == 1 ? import_candidates.items[0] : NULL;
            free(import_candidates.items);
            return resolved;
        }
        free(import_candidates.items);
...
static Sysml2Symbol *resolve_exact_qualified_relationship_endpoint(
    ValidationContext *vctx,
    const char *raw
) {
    if (!vctx || !raw || !strstr(raw, "::")) return NULL;

    const char *last_sep = NULL;
    for (const char *p = raw; p[0] && p[1]; p++) {
        if (p[0] == ':' && p[1] == ':') {
            last_sep = p;
        }
    }
    ...
    size_t parent_len = (size_t)(last_sep - raw);
    size_t name_len = strlen(last_sep + 2);
    char *parent = sysml2_arena_alloc(vctx->symtab->arena, parent_len + 1);
    char *name = sysml2_arena_alloc(vctx->symtab->arena, name_len + 1);
    if (!parent || !name) return NULL;

    memcpy(parent, raw, parent_len);
    parent[parent_len] = '\0';
    memcpy(name, last_sep + 2, name_len + 1);

    Sysml2Scope *target_scope = sysml2_symtab_get_or_create_scope(vctx->symtab, parent);
    Sysml2Symbol *sym = sysml2_symtab_lookup(target_scope, name);
    if (!sym || strcmp(sym->qualified_id, raw) != 0) {
        return NULL;
    }
...
static Sysml2Symbol *resolve_relationship_endpoint(
    ValidationContext *vctx,
    const Sysml2Scope *scope,
    const char *owner_scope,
    const char *raw
    ...
    if (!dot) {
        if (strstr(raw, "::")) {
            return resolve_exact_qualified_relationship_endpoint(vctx, raw);
        }
        return resolve_unique_simple_relationship_endpoint(vctx, scope, owner_scope, raw);
    }

    size_t first_len = (size_t)(dot - raw);
    char *first = sysml2_arena_alloc(vctx->symtab->arena, first_len + 1);
    if (!first) return NULL;
    memcpy(first, raw, first_len);
    first[first_len] = '\0';

    Sysml2Symbol *current = strstr(first, "::")
        ? resolve_exact_qualified_relationship_endpoint(vctx, first)
        : resolve_unique_simple_relationship_endpoint(vctx, scope, owner_scope, first);
    if (!current) return NULL;

    const char *segment = dot + 1;
    while (current && segment && *segment) {
        const char *next_dot = strchr(segment, '.');
        size_t segment_len = next_dot ? (size_t)(next_dot - segment) : strlen(segment);
        char *feature = sysml2_arena_alloc(vctx->symtab->arena, segment_len + 1);
        if (!feature) return NULL;
        memcpy(feature, segment, segment_len);
        feature[segment_len] = '\0';

        Sysml2Scope *feature_scope = sysml2_symtab_get_or_create_scope(vctx->symtab, current->qualified_id);
        current = sysml2_symtab_lookup(feature_scope, feature);
        if (!current || !next_dot) {
            break;
        }
...
static void pass2_resolve_relationships(
    ValidationContext *vctx,
    const SysmlSemanticModel *model
) {
    if (!vctx || !model) return;
    ...
    for (size_t i = 0; i < model->relationship_count; i++) {
        ...
        const Sysml2Scope *scope = sysml2_symtab_get_or_create_scope(vctx->symtab, rel->owner_scope);

        Sysml2Symbol *source = resolve_relationship_endpoint(vctx, scope, rel->owner_scope, rel->source);
        if (source) {
            rel->resolved_source = source->qualified_id;
        }

        Sysml2Symbol *target = resolve_relationship_endpoint(vctx, scope, rel->owner_scope, rel->target);
        if (target) {
            rel->resolved_target = target->qualified_id;
        }
    }
}
...
Sysml2Result sysml2_validate(
    ...
    pass2_resolve_relationships(&vctx, model);
...
Sysml2Result sysml2_validate_multi(
    ...
    for (size_t i = 0; i < model_count; i++) {
        if (models[i]) {
            pass2_resolve_relationships(&vctx, models[i]);
        }
    }
