...
#if defined(_WIN32)
static wchar_t *sysml2_utf8_to_utf16_path(const char *path) {
    ...
    if (MultiByteToWideChar(CP_UTF8, 0, path, -1, wide, length) != length) {
        free(wide);
        return NULL;
    }

    for (wchar_t *cursor = wide; *cursor; cursor++) {
        if (*cursor == L'/') {
            *cursor = L'\\';
        }
    }
...
static char *sysml2_utf16_to_utf8(const wchar_t *path) {
    ...
    if (WideCharToMultiByte(CP_UTF8, 0, path, -1, utf8, length, NULL, NULL) != length) {
        free(utf8);
        return NULL;
    }
...
static char *sysml2_normalize_windows_realpath(char *path) {
    static const char unc_prefix[] = "\\\\?\\UNC\\";
    static const char local_prefix[] = "\\\\?\\";

    char *normalized = path;
    if (strncmp(path, unc_prefix, strlen(unc_prefix)) == 0) {
        size_t suffix_length = strlen(path + strlen(unc_prefix));
        char *unc_path = malloc(suffix_length + 3);
        if (!unc_path) {
            free(path);
            return NULL;
        }
        ...
        unc_path[0] = '\\';
        unc_path[1] = '\\';
        memcpy(unc_path + 2, path + strlen(unc_prefix), suffix_length + 1);
        free(path);
        normalized = unc_path;
    } else if (strncmp(path, local_prefix, strlen(local_prefix)) == 0) {
        size_t suffix_length = strlen(path + strlen(local_prefix));
        char *local_path = malloc(suffix_length + 1);
        if (!local_path) {
            free(path);
            return NULL;
        }

        memcpy(local_path, path + strlen(local_prefix), suffix_length + 1);
        free(path);
...
char *sysml2_get_realpath(const char *path) {
#if defined(_WIN32)
    wchar_t *wide_path = sysml2_utf8_to_utf16_path(path);
    ...
    DWORD capacity = 256;
    wchar_t *wide_resolved = NULL;
    for (;;) {
        wchar_t *next = realloc(wide_resolved, (size_t)capacity * sizeof(wchar_t));
        if (!next) {
            free(wide_resolved);
            CloseHandle(handle);
            return NULL;
    ...
    char *utf8 = sysml2_utf16_to_utf8(wide_resolved);
    free(wide_resolved);
    if (!utf8) return NULL;

    return sysml2_normalize_windows_realpath(utf8);
#else
