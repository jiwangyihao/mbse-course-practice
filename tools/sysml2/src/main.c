...
#if defined(_WIN32)
    char *last_backward = strrchr(path, '\\');
...
static void add_file_directory_to_resolver(Sysml2ImportResolver *resolver, const char *file_path) {
    ...
    if (path_copy) {
        char *last_slash = find_last_path_separator(path_copy);
        if (last_slash) {
            *last_slash = '\0';
...
static Sysml2QueryResult *build_query_result(
    Sysml2PipelineContext *ctx,
    const Sysml2CliOptions *options,
    SysmlSemanticModel **models,
    size_t model_count
) {
    if (!ctx || !options || !models || model_count == 0 || options->select_pattern_count == 0) {
        return NULL;
    }
    ...
    return sysml2_query_execute(patterns, models, model_count, arena);
}

static void write_query_result_output(
    Sysml2PipelineContext *ctx,
    const Sysml2CliOptions *options,
    const Sysml2QueryResult *query_result,
    SysmlSemanticModel **models,
    size_t model_count,
    FILE *out
) {
    if (!ctx || !options || !query_result || !out) {
        return;
    }
    ...
    if (options->list_mode) {
        ...
        sysml2_pipeline_write_query_sysml(ctx, query_result, models, model_count, out);
    }
}
...
static int run_normal_mode(
    ...
    if (input_count == 0) {
        ...
        if (model && final_result == SYSML2_OK) {
            SysmlSemanticModel *stdin_models[1] = {model};
            size_t query_model_count = 0;
            SysmlSemanticModel **query_models = NULL;
            bool query_models_owned = false;

            if (options->select_pattern_count > 0) {
                query_models = sysml2_resolver_get_all_models(ctx->resolver, &query_model_count);
                if (query_models && query_model_count > 0) {
                    query_models_owned = true;
                } else {
                    query_models = stdin_models;
                    query_model_count = 1;
                }
            }

            if (options->select_pattern_count > 0) {
                Sysml2QueryResult *query_result = build_query_result(
                    ctx, options, query_models, query_model_count
                );
                if (query_result) {
                    FILE *out = options->output_file ? fopen(options->output_file, "w") : stdout;
                    if (out) {
                        write_query_result_output(
                            ctx, options, query_result, query_models, query_model_count, out
                        );
                        if (options->output_file) fclose(out);
                    }
                }
                if (query_models_owned) {
                    free(query_models);
                }
        ...
        if (!has_parse_errors && input_models[0]) {
            ...
            if (options->select_pattern_count > 0) {
                ...
                if (query_models && query_model_count > 0) {
                    ...
                    query_models = input_models;
                    query_model_count = input_count;
                }
            }
