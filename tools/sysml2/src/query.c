...
Sysml2QueryResult *sysml2_query_execute(
    ...
    for (size_t m = 0; m < model_count; m++) {
        ...
        for (size_t i = 0; i < model->relationship_count; i++) {
            ...
            /* Prefer proven resolved endpoints; fall back to lexical raw text */
            const char *source_id = rel->resolved_source ? rel->resolved_source : rel->source;
            const char *target_id = rel->resolved_target ? rel->resolved_target : rel->target;

            if (!source_id || !target_id) continue;

            bool source_in = sysml2_query_result_contains(result, source_id);
            bool target_in = sysml2_query_result_contains(result, target_id);
