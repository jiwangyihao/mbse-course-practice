...
// @ts-expect-error test-only import of JS sidecar seam
import { runSysml2Analysis } from '../sidecar/sysml2-backend.mjs';
import { defaultTianwen2ConfirmedData, extractTianwen2ConfirmedData, validateViewModel } from '../src/domain/modelGeneration';
import { generateTianwen2ModelArtifacts, getModelSourceEntryContent } from '../src/domain/modelGeneration.node';
...
function extractSysmlDefinitionBlock(sysmlText: string, definitionKind: string, identifier: string) {
  const pattern = new RegExp(`\\b${definitionKind}\\s+(?:def\\s+)?${identifier}\\s*\\{`, 'i');
  const match = pattern.exec(sysmlText);
  if (!match) {
    return '';
  }

  const braceStart = sysmlText.indexOf('{', match.index);
  if (braceStart < 0) {
    return '';
  }

  let depth = 0;
  for (let index = braceStart; index < sysmlText.length; index += 1) {
    const current = sysmlText[index];
    if (current === '{') {
      depth += 1;
    } else if (current === '}') {
      depth -= 1;
      if (depth === 0) {
        return sysmlText.slice(braceStart + 1, index);
      }
    }
  }
...
function entrySysmlText(artifacts: { sourceSet: { rootDir: string; entryPath: string; files: Array<{ path: string; content: string }> } }) {
  return getModelSourceEntryContent(artifacts.sourceSet);
}

function sourceFileText(
  artifacts: { sourceSet: { files: Array<{ path: string; content: string }> } },
  relativePath: string,
) {
  const file = artifacts.sourceSet.files.find((entry) => entry.path === relativePath);
  if (!file) {
    throw new Error(`source set 缺少文件：${relativePath}`);
  }
  return file.content;
}

async function materializeSourceSet(
  workspaceRoot: string,
  sourceSet: { rootDir: string; entryPath: string; files: Array<{ path: string; content: string }> },
) {
  const sourceRoot = sourceSet.rootDir ? path.join(workspaceRoot, sourceSet.rootDir) : workspaceRoot;
  await Promise.all(
  ...
  return {
    sourceRoot,
    entryAbsolutePath: path.join(sourceRoot, sourceSet.entryPath),
  };
}
...
describe('天问二号确认数据领域模型生成契约', () => {
  it('从内置材料使命目标抽取真实候选使命而不是 Markdown 标题', async () => { const confirmedData = extractTianwen2ConfirmedData([
    '# 天问二号任务与需求材料',
    '',
  ...
  it('从确认数据生成 SysML v2 文本、JSON 视图模型、需求视图和 BDD', async () => { const artifacts = await generateTianwen2ModelArtifacts(confirmedTianwen2Data)
  
  const sysmlText = entrySysmlText(artifacts);
  const requirementsSysml = sourceFileText(artifacts, 'requirements.sysml');
  const structureSysml = sourceFileText(artifacts, 'structure.sysml');
  expect(sysmlText, '入口 SysML 必须使用稳定包名承载确认后的领域模型').toContain(
    'package Tianwen2ConfirmedModel',
  );
  expect(requirementsSysml, '需求定义必须位于 requirements.sysml，而不是仅存在入口 metadata 文件').toContain('requirement def');
  expect(structureSysml, '部件定义必须位于 structure.sysml，供 BDD 结构视图引用').toContain('part def');
  expect(`${requirementsSysml}\n${structureSysml}`).toContain('REQ-TW2-001');
  expect(`${requirementsSysml}\n${structureSysml}`).toContain('REQ-TW2-004');
  expect(structureSysml).toContain('测控通信分系统');
  expect(
    structureSysml,
    'SysML source set 应保留需求到分系统的满足或追溯关系',
  ...
  it('从确认数据生成活动图视图并用活动流连接已声明行为节点', async () => { const artifacts = await generateTianwen2ModelArtifacts(confirmedTianwen2Data)
  ...
  it('从确认数据生成 IBD 视图并表达部件、端口和连接', async () => { const artifacts = await generateTianwen2ModelArtifacts(confirmedTianwen2Data)
  ...
  const sysmlWithoutDocs = sourceFileText(artifacts, 'structure.sysml').replace(/\bdoc\s*\/\*[\s\S]*?\*\//gi, '');
  const portDefinitions = sysmlWithoutDocs.match(/\bport\s+def\s+[A-Za-z_][\w]*/g) ?? [];
  expect(portDefinitions.length, 'structure.sysml 必须声明可机读 port def，不能只在 doc 注释里写端口类型名称').toBeGreaterThanOrEqual(3);
  expect(sysmlWithoutDocs, 'structure.sysml 必须声明样品转移端口类型定义，供 IBD 端口用类型引用').toMatch(/\bport\s+def\s+sample_transfer\b/i);
  ...
  expect(
    ...
    'part 内必须用 port 声明把 sample_transfer_out 绑定到 sample_transfer 端口类型，而不是只靠说明文字',
  ).toMatch(/\bport\s+sample_transfer_out\s*:\s*sample_transfer\s*;/i);
  
  const systemContextBlock = extractSysmlDefinitionBlock(sysmlWithoutDocs, 'part', 'system_context');
  expect(systemContextBlock.trim(), 'system_context 必须包含非注释的命名 connection usage').not.toBe('');
  expect(
    systemContextBlock,
    'IBD 连接必须用命名 connection usage + connect 语法表达端点，不能退回无效的 end source/end target 连接定义',
  ).toMatch(/\bconnection\s+connection_sample_transfer_to_platform\s+connect\b/i);
  expect(systemContextBlock, '样品转移连接块必须显式引用源端口 sample_transfer_out').toMatch(/\bsample_transfer_out\b/i);
  expect(systemContextBlock, '样品转移连接块必须显式引用目标端口 sample_return_mechanical_interface').toMatch(/\bsample_return_mechanical_interface\b/i);
  ...
  it('生成的 SysML 保留活动流、约束、参数与绑定语义 ID', async () => { const artifacts = await generateTianwen2ModelArtifacts(confirmedTianwen2Data)
  const behaviorSysml = sourceFileText(artifacts, 'behavior.sysml').replace(/\bdoc\s*\/\*[\s\S]*?\*\//gi, '');
  const constraintsSysml = sourceFileText(artifacts, 'constraints.sysml').replace(/\bdoc\s*\/\*[\s\S]*?\*\//gi, '');
  
  expect(behaviorSysml, '活动流必须映射为 sysml2 接受的 action/flow 关系，而不是直接删掉行为衔接').toMatch(
    /action def mission_sequence[\s\S]*flow activity_sample_return_step to activity_cruise_safety_step;[\s\S]*flow activity_cruise_safety_step to activity_telemetry_downlink_step;[\s\S]*flow activity_telemetry_downlink_step to activity_trace_coverage_check_step;/i,
  );
  expect(constraintsSysml, '每个已确认约束 ID 都必须保留在 strict SysML 中').toMatch(/\bconstraint def constraint_mass_budget\b/i);
  expect(constraintsSysml).toMatch(/\bconstraint def constraint_power_budget\b/i);
  expect(constraintsSysml, '每个已确认参数 ID 都必须保留在 strict SysML 中').toMatch(/\battribute def spacecraft_dry_mass\b/i);
  expect(constraintsSysml).toMatch(/\battribute def solar_array_output\b/i);
  expect(constraintsSysml, '每个已确认绑定 ID 与其 constraintId/parameterId 端点都必须保留在 strict SysML 中').toMatch(
    /\bbinding binding_mass_budget_dry_mass of constraint_mass_budget = spacecraft_dry_mass;/i,
  );
  expect(constraintsSysml).toMatch(
    /\bbinding binding_power_budget_solar_array_output of constraint_power_budget = solar_array_output;/i,
  ); });

  it('生成器输出的多文件 SysML source set 可被 vendored sysml2 严格接受', async () => {
    const artifacts = await generateTianwen2ModelArtifacts(confirmedTianwen2Data)
    const workspaceRoot = await mkdtemp(path.join(os.tmpdir(), 'mbse-generated-sysml-'));
    const { sourceRoot, entryAbsolutePath } = await materializeSourceSet(workspaceRoot, artifacts.sourceSet);
    ...
    try {
      ...
      expect(analysis.valid, analysis.diagnostics.map((diagnostic: { message: string }) => diagnostic.message).join('\n')).toBe(true);
      expect(analysis.backend).toBe('sysml2');
      expect(analysis.diagnostics.filter((diagnostic: { severity: number }) => diagnostic.severity <= 2)).toEqual([]);
    } finally {
      await rm(workspaceRoot, { recursive: true, force: true });
    }
  }, 120_000);

  it('内置样例 SysML source set 与生成器当前输出保持同步', async () => {
    const artifacts = await generateTianwen2ModelArtifacts(defaultTianwen2ConfirmedData)

    await Promise.all(artifacts.sourceSet.files.map(async (file) => {
      const sampleSysml = await readFile(`sample-projects/tianwen-2/model/${file.path}`, 'utf8');
      expect(sampleSysml.trimEnd()).toBe(file.content.trimEnd());
    }));
  });

  it('从确认数据生成需求追溯矩阵并表达结构列、行为列与覆盖状态', async () => { const artifacts = await generateTianwen2ModelArtifacts(confirmedTianwen2Data)
  const views = artifacts.viewModel.views as unknown as IssueFiveView[];
  const activityView = views.find((view) => view.kind === 'activity');
  ...
  it('校验器为追溯矩阵中的未覆盖需求返回可定位 finding', async () => { const uncoveredViewModel = {
    schemaVersion: '0.3.0',
  ...
  it('校验器报告 IBD 端口引用缺失、连接端点缺失和接口不完整', async () => { const invalidViewModel = {
    schemaVersion: '0.4.0',
  ...
  it('校验器发现同一 IBD 连接在 edges 与 connections 双写后发生端口漂移', async () => { const driftedViewModel = {
    schemaVersion: '0.4.0',
  ...
  it('校验器报告参数约束视图中的参数缺失', async () => { const artifacts = await generateTianwen2ModelArtifacts(confirmedTianwen2Data)
  ...
  it('校验器报告参数约束视图中的单位缺失', async () => { const artifacts = await generateTianwen2ModelArtifacts(confirmedTianwen2Data)
  const invalidViewModel = structuredClone(artifacts.viewModel);
  const parameterView = invalidViewModel.views.find((view) =>
    /parameter-constraints|参数约束/i.test(`${view.kind} ${view.id} ${view.title}`),
  );
  ...
  it('校验器报告参数约束视图中的绑定缺失', async () => { const artifacts = await generateTianwen2ModelArtifacts(confirmedTianwen2Data)
  const invalidViewModel = structuredClone(artifacts.viewModel);
  const parameterView = invalidViewModel.views.find((view) =>
    /parameter-constraints|参数约束/i.test(`${view.kind} ${view.id} ${view.title}`),
  );
  ...
  it('校验器报告单个参数约束未绑定任何参数', async () => { const artifacts = await generateTianwen2ModelArtifacts(confirmedTianwen2Data)
  const invalidViewModel = structuredClone(artifacts.viewModel);
  const parameterView = invalidViewModel.views.find((view) =>
    /parameter-constraints|参数约束/i.test(`${view.kind} ${view.id} ${view.title}`),
  );
  ...
  it('校验器报告参数约束视图中的相关模型元素缺失', async () => { const artifacts = await generateTianwen2ModelArtifacts(confirmedTianwen2Data)
  const invalidViewModel = structuredClone(artifacts.viewModel);
  const parameterView = invalidViewModel.views.find((view) =>
    /parameter-constraints|参数约束/i.test(`${view.kind} ${view.id} ${view.title}`),
  );
  ...
  it('校验器报告 schema 错误和缺失引用', async () => { const invalidViewModel = {
    projectId: 'tianwen-2',
    views: [
      {
        id: 'requirements-view',
